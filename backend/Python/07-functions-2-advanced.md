...
- [3. Paramètre (Parameter) et Argument (Argument)](#3-paramètreparameter-et-argumentargument)
    - [3.1. Arguments nommés (Keyword Arguments)](#31-arguments-nommés-keyword-arguments)
...
### 3.1-Arguments nommés (Keyword Arguments)
Pour éviter de se tromper sur l'ordre dans lequel on passe les arguments aux paramètres, il est possible d'utiliser des arguments nommés de la façon suivante :

```py
ma_fonction(name=value)
```
- Par exemple : 
```py
def salutation(nom, age):
...
salutation(age=21, nom="Dave") # De cette manière, peu importe l'ordre, il ne devrait pas y avoir de problème. 
```

> Note : Lorsqu'on utilise des arguments nommés, il ne faut pas mettre d'espace entre les assignations `=` selon la convention PEP 8
